...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, down=True, activation='relu', use_dropout=False):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 4, 2, padding=1, bias=False, padding_mode='reflect') if down \
            else nn.ConvTranspose2d(in_channels, out_channels, 4, 2, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU() if activation else nn.LeakyReLU(0.2)
        )
...
class Generator(nn.Module):
    def __init__(self, in_channels, features=64):
        ...
        self.up4 = Block(features*8*2, features*8, down=False, activation='relu', use_dropout=True)
        self.up5 = Block(features*8*2, features*4, down=False, activation='relu', use_dropout=True)
        self.up6 = Block(features*4*2, features*2, down=False, activation='relu', use_dropout=True)
        self.up7 = Block(features*2*2, features, down=False, activation='relu', use_dropout=True)
        self.final_up = nn.Sequential(nn.ConvTranspose2d(features*2, in_channels, 4, 2, 1),
                                          nn.Tanh())
